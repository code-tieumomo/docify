...
const { data } = await useAsyncData("articles", () => queryContent().where({ category: { $eq: slug } }).sort({ date: -1 }).find());
</script>
...
<template>
  <section>
    ...
    <p class="mt-2 text-gray-600">
      Danh sách tất cả các bài viết thuộc danh mục <strong>"{{ slug }}"</strong>
    </p>
